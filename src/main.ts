...
function fallBlossoms() {
  ...
  const count = window.innerWidth < 800 ? 10 : 16
  const palette = ['#e37a93', '#f3a3b4', '#de6b86', '#f08aa6']
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const size = isFlower ? 18 + Math.random() * 12 : 12 + Math.random() * 14
    el.style.width = `${size}px`
    el.style.color = palette[i % palette.length]
    el.style.opacity = String(0.45 + Math.random() * 0.28)
    el.style.setProperty('--x', `${Math.random() * 100}vw`)
    el.style.setProperty('--dx', `${(Math.random() - 0.4) * 180}px`)
    el.style.setProperty('--dur', `${11 + Math.random() * 8}s`)
    el.style.setProperty('--delay', `${-Math.random() * 16}s`)
    el.style.setProperty('--spin', `${220 + Math.random() * 400}deg`)
